...
from apps.workflow.action_sla import is_action_overdue, mark_action_sla_breach_if_needed
...
def check_action_sla_breaches():
    """Notify responsible users when workflow action SLA is missed."""
    from apps.notifications.services import notify_action_sla_breach
    ...
    designs = DesignRequest.objects.filter(status__in=active_statuses)
    notified = 0
    for design in designs.iterator():
        if not is_action_overdue(design):
            continue
        if mark_action_sla_breach_if_needed(design):
            notify_action_sla_breach(design)
            notified += 1
    return notified
...
@shared_task(name='apps.workflow.tasks.check_action_sla_breaches')
def check_action_sla_breaches_task():
    return check_action_sla_breaches()
